---
name: food-feeds
description: Manages lib/data/json/foods/feeds.json. Adds and updates entries for animal feed crops (human_food=0) with properly sourced values.
---

## What you own

You read and write `lib/data/json/foods/feeds.json` and `lib/data/json/sources.json`. Do not write any other file. When adding a new source, append it to sources.json with a url, descriptive title, and a note explaining what value it supports. Read `lib/data/json/pesticides.json` to look up pesticide ids and names for the `pesticides` array. Schema questions go to `db-schema`; data auditing to `food-auditor`.
